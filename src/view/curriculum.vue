<template>
  <div>
    ...
    <!-- banner开始 -->
    <banner-common categoryType=7 authCode="study"
    ...
    <div class="publicSearch">
      <div class="wrapper clearfix especialback">
        ...
        <div class="searchInput fr">
          <input type="text" class="fl searchInputName" placeholder="请输入名称" v-model="curriculumForm.coursewareName">
          <el-select v-model="curriculumForm.learnStatus" placeholder="状态" class="fl" :clearable=true>
            <el-option
          ...
          <div class="searchBtn cursorPointer fl" @click="getcurriculumList">
      ...
      <div class="wrapper">
        <div class="examTabTit" >
          <ul class="examTabTitList fl">
            <li v-bind:class="{examTabTitListSelected : this.curriculumForm.cswType == 1}" @click="tabClick(1)">线上课程</li>
            <li v-bind:class="{examTabTitListSelected : this.curriculumForm.cswType == 2}" @click="tabClick(2)">线下课程</li>
          </ul>
        ...
        <div class="classifyTabContent">
          <ul class="tabContentList clearfix">
            ...
            <li v-for="openClass in curriculumList" class="cursorPointer">
              <div class="tabContentImg" @click='goPublicDetail(openClass.id)'>
                <img :src="openClass.imageUrl" alt="">
                <div class="everyOneDetail" v-if="curriculumForm.cswType == 1">
    ...
    <div style="margin:82px 0">
      <page v-on:castCurrentPage="getCurrentPage" :pages = "curriculumPage"></page>
    </div>
    <!--分页结束-->
    <!-- 底部开始 -->
...
export default {
  ...
  methods:{
    getCurrentPage(val){
      this.curriculumPage.currentPage = val;
    ...
    setOrderStrategy(val){
      ...
      this.curriculumForm.orderStrategy = val;
      this.getcurriculumList();
    },
    setCategoryCode(val){
      this.curriculumForm.categoryCodeList = [];
      this.curriculumForm.categoryCodeList.push(val)
      this.getcurriculumList();
    },
    tabClick(val){
      this.curriculumForm.cswType = val;
      this.getcurriculumList();
    },
    getcurriculumList(){
      httpTool.apiHttp({
        url: cast.apiUrl_lms + "/centerLearnCurriculum/queryCurriculum" ,
        method: "post",
        params: this.curriculumForm,
        page:   this.curriculumPage,
        cbfn: (response) => {
          if(response.code == cast.returnState().BIZ_SUCCESS){
            this.curriculumPage = response.data.page;
            this.curriculumList = response.data.data;
            for(var i=0; i<this.curriculumList.length;i++){
              this.curriculumList[i].imageUrl = cast.apiUrl_lms + "/file/getImage/" + this.curriculumList[i].coverImageId;
            }
          }
        }
      });
    ...
    goPublicDetail(id){
      if(this.curriculumForm.cswType === 1){
        this.$router.push({path:'/curriculumDetail/' + id});
      }
    },
  },
  mounted(){
    this.curriculumForm.cswType = 1;//线上
